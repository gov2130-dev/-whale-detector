...
st.title("V99 - عقود مرتبة من الأقوى")
st.caption(f"الرياض {datetime.now(RIYADH).strftime('%H:%M:%S')} | نيويورك {datetime.now(NY).strftime('%H:%M')} | {'LIVE' if is_market_open() else 'PRE'}")

c1,c2=st.columns(2)
with c1:
    if st.button("📨 اختبار تلجرام", type="primary"):
        st.success("انرسل") if send("✅ V99 شغال") else st.error("بدل التوكن")
with c2:
...
sent=load()
if st.button("🔍 افحص 54 - الأقوى أول", type="primary"):
    prog=st.progress(0)
    candidates=[]
    ...
    for idx, (score, chg, t, direction, reason) in enumerate(candidates):
        ...
        time.sleep(0.2)
    st.balloons()
    st.info(f"تم: CALL {call_c} | PUT {put_c} | الأقوى أول")
