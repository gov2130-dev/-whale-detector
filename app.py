...
st.set_page_config(layout="wide", page_title="Whale V19.1 Fixed", initial_sidebar_state="expanded")
...
.time-new {background:#22c55e; color:#fff; padding:3px 6px; border-radius:10px; font-size:10px;}
.time-old {background:#64748b; color:#fff; padding:3px 6px; border-radius:10px; font-size:10px;}
</style>
""", unsafe_allow_html=True)

st.title("🐋 Whale V19.1 - مصحح + آخر ساعة")
...
min_prem=st.sidebar.slider("💰 اقل حوت", 500000, 5000000, 1000000, 250000, key="min19f")
time_filter=st.sidebar.select_slider("⏰ فلتر الوقت", options=["آخر 15 دقيقة","آخر ساعة","آخر 3 ساعات","اليوم كامل"], value="آخر ساعة", key="tf19f")
bell_on=st.sidebar.checkbox("🔔 جرس حوت جديد", True, key="bell19f")
smart_filter=st.sidebar.checkbox("🧠 اتجاه واحد لكل شركة", True, key="sm19f")
auto=st.sidebar.checkbox("⚡ فحص تلقائي", True, key="au19f")

st.sidebar.write(f"فحص {st.session_state.current_idx}/{len(get_tickers())} | حيتان {len(st.session_state.results)}")
if st.sidebar.button("⏰ آخر ساعة فقط", key="b0_19f"): st.session_state.page="LASTHOUR"
if st.sidebar.button("🔥 آخر 15 دقيقة", key="b1_19f"): st.session_state.page="15MIN"
if st.sidebar.button("🏆 اقوى 10", key="b2_19f"): st.session_state.page="TOP10"
if st.sidebar.button("📋 كل اليوم", key="b3_19f"): st.session_state.page="ALL"
if st.sidebar.button("🔔 جرب الجرس", key="btest19f"): play_bell(times=2)
if st.sidebar.button("🔄 مسح", key="b7_19f"): st.session_state.results=pd.DataFrame(); st.session_state.current_idx=0; st.session_state.new_whales=[]; st.session_state.sent=set(); st.rerun()
...
if auto and st.session_state.current_idx < len(all_tickers):
    ...
    st.info(f"🔴 LIVE يفحص {start}-{end} | فلتر: آخر {mins} دقيقة")
    new_rows=[]
            ...
            for typ, df in [("CALL BUY", chain.calls), ("PUT SELL", chain.puts)]:
                ...
                for _, r in f.iterrows():
                    ...
                    new_rows.append({
                        ...
                        "exp":exp,"lastTrade":ltd,"minutes_ago":minutes_ago
                    })
        except:
            pass
    if new_rows:
        ...
        if fresh and bell_on:
            ...
            if biggest>=2000000: play_bell(times=2); st.toast(f"🔔 حوت جديد ${biggest/1e6:.2f}M", icon="🐋")
...
if not st.session_state.results.empty:
    ...
    if final_time.empty:
        st.warning(f"⏳ لا يوجد حيتان في آخر {mins} دقيقة - السوق مقفل ويكند. اختار اليوم كامل")
    ...
    if st.session_state.new_whales:
        ...
        for i, w in enumerate(st.session_state.new_whales[:3]):
            with cols[i % len(cols)]:
                ...
                st.markdown(f"<div style='background:#dcfce7; padding:10px; border-radius:12px; text-align:center; border:2px solid #22c55e;'><b>🔔 {w['ticker']} جديد!</b><br>{w['signal']}<br>${w['opt_price']:.2f} | ${w['premium_M']:.2f}M<br>قبل {txt}</div>", unsafe_allow_html=True)
        if st.button("✖️ اخفاء", key="hide19f"): st.session_state.new_whales=[]; st.rerun()
    ...
    def build_html(df):
        ...
        for _, w in df.iterrows():
            ...
            elif ago<=60: time_badge=f'<span class="time-new">⏰ {ago} د</span>'
            else: time_badge=f'<span class="time-old">{ago//60} س</span>'
...
st.caption(f"Last {datetime.now().strftime('%H:%M:%S')} | V19.1 Fixed - مصحح")        requests.post(f"https://api.telegram.org/bot{token}/sendMessage", data={"chat_id":chat,"text":msg}, timeout=5)
        return True
    except: return False
...
st.caption(f"Last {datetime.now().strftime('%H:%M:%S')} | V19 Last Hour Filter | الجرس بدون كلام")        requests.post(f"https://api.telegram.org/bot{token}/sendMessage", data={"chat_id":chat,"text":msg}, timeout=5)
        return True
    except: return False
