...
st.set_page_config(layout="wide", page_title="Whale V20 One Alert", initial_sidebar_state="expanded")
...
st.title("🐋 Whale V20 - تنبيه واحد لكل شركة")
...
st.caption(f"Last {datetime.now().strftime('%H:%M:%S')} | V20 One Alert Per Ticker")        return True
    except: return False
...
st.caption(f"Last {datetime.now().strftime('%H:%M:%S')} | V19.1 Fixed - مصحح")
