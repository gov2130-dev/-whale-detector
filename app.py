import streamlit as st, yfinance as yf, pandas as pd, time, io, requests
...
def create_fajer_card(ticker, o_type, strike, entry, stop, company):
    ...
    draw.text((W//2,70), "الامريكي تحت الفجر", fill=(110,255,255), font=font_big, anchor="mm")
    draw.text((W//2,150), "BROADCOM STYLE", fill=(0,255,100), font=font_med, anchor="mm")
    ...
    draw.text((50,y0+140), f"نوع العقد {o_type}", fill=(0,255,100), font=font_small)
    draw.text((50,y0+200), f"السترايك {strike}", fill=(255,255,255), font=font_small)
    draw.text((50,y0+260), f"الانتهاء WEEKLY", fill=(200,200,200), font=font_small)
    draw.text((50,y0+315), f"ذهبي 6/7", fill=(255,215,0), font=font_small)
    ...
    draw.text((580,y0+15), "الاهداف", fill=(0,240,255), font=font_med)
    t1=entry*1.5
    targets=[t1, t1*1.07, t1*1.15, t1*1.22, t1*1.30, t1*1.40, t1*1.50, t1*1.65, t1*1.80]
    for i,t in enumerate(targets):
        yy=y0+80+i*82
        draw.text((580,yy), f"{i+1} -> {t:.2f}", fill=(0,255,150), font=font_small)

    draw.text((W//2, H-100), "TrkHr Trading", fill=(0,240,255), font=font_big, anchor="mm")
    draw.text((W//2, H-50), "ليست توصية بيع او شراء", fill=(100,100,100), font=font_small, anchor="mm")
...
st.markdown(f'<div class="time">● {ksa_str} KSA | {CHAT_ID} | V69.1 مصحح ✅ | فجر {"شغال" if is_fajer else "ينتظر"}</div>', unsafe_allow_html=True)
...
with c2:
    if st.button("📸 اختبار صورة"):
        buf,_=create_fajer_card("AVGO","CALL",300,365.0,360.0,"BROADCOM")
        if send_photo_tg(buf, "🔥 اختبار V69.1\nAVGO 300 CALL\nدخول 365 وقف 360"):
            st.success("✅ انرسلت - شف تلجرام"); st.image(buf)
        else: st.error("❌ فشل")
    ...
    for i,ticker in enumerate(tickers):
        ...
        try:
            ...
            for _, r in df.iterrows():
                ...
                if sp>18: continue
                picked=(r,bid,ask,sp,vol); break
            if not picked: continue
            r,bid,ask,sp,vol=picked; strike=int(r['strike'])
            if abs((strike-curr)/curr*100)>7: continue
            ok=0
            if vol>=200: ok+=1
            if (trend=="BULL" and rsi<=42) or (trend=="BEAR" and rsi>=63): ok+=1
            if (trend=="BULL" and curr < vwap) or (trend=="BEAR" and curr > vwap): ok+=1
            if (trend=="BULL" and ema9 < ema21) or (trend=="BEAR" and ema9 > ema21): ok+=1
            if vol_ratio>=1.1: ok+=1
            if sp<=12: ok+=1
            ok+=1
            if ok<5: continue
            entry=ask; sl=entry*0.60; company=COMPANY_NAMES.get(ticker, ticker)
            key=f"{ticker}{strike}{trend}{ok}"
            if key not in st.session_state.sent:
                ...
                cap=f"{'🔥🔥 ذهبي 6/7' if ok>=6 else '🔔 جيد 5/7'} {ksa_str}\n{ticker} {strike} {'PUT' if trend=='BEAR' else 'CALL'}\nدخول ${entry:.2f} وقف ${sl:.2f}\n🎯1 ${targets[0]:.2f}"
                if send_photo_tg(buf, cap):
                    st.session_state.sent.add(key); st.image(buf, caption=f"{ticker} ارسل")
            time.sleep(0.2)
        except Exception as e:
            st.write(f"خطأ {ticker}: {e}"); continue
