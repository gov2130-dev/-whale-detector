...
st.set_page_config(layout="wide", page_title="Whale V29.1 No Flicker", initial_sidebar_state="expanded")
...
st.title("🐋 Whale V29.1 - نفس V29 المنطقي بس بدون بهوت 🔥💰")
...
st.sidebar.markdown("## 🎯 V29.1 - نفس V29 بدون بهوت")

st.sidebar.markdown("### 🔥 نمط التداول")
trade_mode=st.sidebar.select_slider("🎯 نمطك؟", options=["سكالبينج 0DTE (دبلات سريعة)","سوينغ 3-14 يوم (آمن)","الكل - أعرض الفرصتين"], value="الكل - أعرض الفرصتين", key="mode291")

st.sidebar.markdown("### 💰 فلاتر")
min_prem=st.sidebar.slider("💰 أقل حوت", 100000, 5000000, 500000, 100000, key="m291")
min_vol=st.sidebar.slider("📊 أقل VOL", 1, 100000, 5000, 1000, key="v291")

exp_filter=st.sidebar.select_slider("📅 الانتهاء", options=["الكل (0DTE يعطي دبلات)","اليوم و بكرة فقط (0-1 يوم)","3-14 يوم (سوينغ)","استبعد المنتهي فقط"], value="الكل (0DTE يعطي دبلات)", key="exp291")
opt_price_filter=st.sidebar.select_slider("💵 سعر الأوبشن", options=["الكل","$0.2-$2 (0DTE رخيص = دبل)","$0.5-$5 (ممتاز)","$0.5-$3"], value="$0.2-$2 (0DTE رخيص = دبل)" if "0DTE" in trade_mode else "$0.5-$5 (ممتاز)", key="opt291")
time_filter=st.sidebar.select_slider("⏰ متى دخل", options=["اليوم كامل","آخر 3 ساعات","آخر ساعة (لحظي)","آخر 15 دقيقة (0DTE)"], value="آخر 15 دقيقة (0DTE)" if "0DTE" in trade_mode else "آخر ساعة (لحظي)", key="tf291")
score_filter=st.sidebar.select_slider("⭐ أقل سكور", options=["الكل","⭐+ (3+)","⭐⭐+ (5+) قوي","⭐⭐⭐ فقط (7+)"], value="⭐+ (3+)", key="score291")

# حل البهوت: خليت الفحص كل دقيقة مو 15 ثانية + بدون جرس
refresh_interval=st.sidebar.select_slider("⏱️ فحص كل كم؟ (حل البهوت)", options=["1 دقيقة","2 دقيقة","5 دقائق","يدوي فقط"], value="1 دقيقة", key="int291")
map_sec={"1 دقيقة":60,"2 دقيقة":120,"5 دقائق":300,"يدوي فقط":9999}
interval_sec=map_sec[refresh_interval]

auto=st.sidebar.checkbox("⚡ فحص تلقائي (ثابت بدون بهوت)", True, key="a291")

st.sidebar.markdown("---")
st.sidebar.markdown("### 📊 عرض الجداول")
if st.sidebar.button("🔥 0DTE اليوم - دبلات", key="b0_291"): st.session_state.page="0DTE"; st.rerun()
if st.sidebar.button("⚡ آخر 15 دقيقة", key="b15_291"): st.session_state.page="15MIN"; st.rerun()
if st.sidebar.button("🏆 أقوى 20", key="b20_291"): st.session_state.page="TOP20"; st.rerun()
if st.sidebar.button("💰 دبلات محتملة", key="bDouble_291"): st.session_state.page="DOUBLE"; st.rerun()
if st.sidebar.button("📋 كل اليوم", key="bAll_291"): st.session_state.page="ALL"; st.rerun()

st.sidebar.markdown("---")
if st.sidebar.button("🔄 فحص الآن", key="bNow_291"): st.session_state.current_idx=0; st.rerun()
if st.sidebar.button("🗑️ مسح", key="bClear_291"): st.session_state.results=pd.DataFrame(); st.session_state.current_idx=0; st.session_state.new_whales=[]; st.session_state.sent=set(); st.rerun()
...
def calc_logical_score(row, stock_data):
    ...
    sd["double_potential"]="💰 دبل 100-300%" if is_0dte and abs((stock_data["price"] if stock_data else row["strike"]) - row["strike"]) / (stock_data["price"] if stock_data else 1) <0.03 else f"هدف +{40 if days<=7 else 30}%"
...
# ===== عرض الجدول أولا - ثابت بدون بهوت =====
if not st.session_state.results.empty:
    ...
    if st.session_state.new_whales:
        st.markdown("### 🔔 قرار منطقي دخل الآن - بدون بهوت")
        ...
        if st.button("✖️ اخفاء", key="hide291"): st.session_state.new_whales=[]; st.rerun()

    dte_count=len(final[final["is_0dte"]==True]) if not final.empty and "is_0dte" in final.columns else 0
    st.success(f"✅ V29.1 ثابت بدون بهوت | النمط: {trade_mode} | 0DTE: {dte_count} | سوينغ: {len(final)-dte_count} | {len(final)} حوت | آخر فحص {st.session_state.last_refresh.strftime('%H:%M:%S')}")
    ...
    st.warning("⏳ اضغط فحص الآن - V29.1 بدون بهوت")

# ===== فحص في الخلفية - بدون بهوت =====
all_tickers=get_tickers()
if auto and interval_sec<9999 and st.session_state.current_idx < len(all_tickers):
    start=st.session_state.current_idx; end=min(start+4, len(all_tickers))
    # شلت progress bar المتحرك اللي كان يسبب بهوت
    st.caption(f"🔴 يفحص {all_tickers[start:end]} في الخلفية... ({st.session_state.current_idx}/{len(all_tickers)}) - بدون بهوت")
    ...
    if st.session_state.current_idx>=len(all_tickers):
        ...
        time.sleep(1.2) # أبطأ عشان ما يبهت
        st.rerun()
else:
    if st.session_state.current_idx>=len(all_tickers): st.session_state.current_idx=0

st.caption(f"Last {st.session_state.last_refresh.strftime('%H:%M:%S')} | V29.1 نفس V29 بس بدون بهوت | 0DTE = دبلات منطقي | الثلاثاء")
