...
st.set_page_config(layout="wide", page_title="V104 FINAL")
...
def find_anytime_style(ticker):
    try:
        curr, tk = get_data(ticker)
        if curr==0: return []
        today=datetime.now(NY).date()
        try: exps=[e for e in tk.options if 0 <= (datetime.strptime(e,"%Y-%m-%d").date()-today).days <= 45][:5]
        except: return []
        res=[]
        for exp in exps:
            ...
            for direction, opts in [("CALL", chain.calls), ("PUT", chain.puts)]:
                ...
                for _, r in opts.iterrows():
                    try:
                        ...
                        # يوم السبت lastPrice يصير 0 نستخدم bid كبديل عشان يطلع بأي وقت
                        last=float(r.get('lastPrice',0) or 0)
                        bid=float(r.get('bid',0) or 0)
                        if last==0: last=bid
                        if last==0: last=float(r.get('ask',0) or 0) * 0.9
                        if last==0 or last<0.3: continue
                        if not (0.3 <= last <= 6.0): continue
                        strike=float(r['strike'])
                        bw=abs(strike-curr)/curr*100
                        if bw>4.0: continue  # نوسع BW لـ 4% عشان يطلع QCOM مثلك
                        if oi<500: continue  # ننزل OI لـ 500 عشان يضمن يطلع بأي وقت
                        score = (oi if oi>8000 else oi*0.5) + (5-bw)*100
                        res.append((score, {"ticker":ticker,"curr":curr,"exp":exp,"days":days,"strike":strike,"last":last,"bid":bid,"vol":int(r.get('volume',0) or 0),"oi":oi,"type":direction,"bw":round(bw,2)}))
                    except: continue
            time.sleep(0.3)
        return res
    except: return []

def build_msg_old_style(c, reason):
...
st.title("V104 FINAL - بأي وقت + نفس الاستايل")
st.caption(f"الرياض {datetime.now(RIYADH).strftime('%H:%M:%S')} | يبحث بأي وقت 0-45 يوم | OI>500 BW<4% 0.3-6$")
col1,col2=st.columns(2)
with col1:
    if st.button("📨 اختبار تلجرام"): st.success("✅") if send("✅ V104 FINAL شغال بنفس الاستايل") else st.error("فشل")
with col2:
...
if st.button("🔍 افحص 54 - بأي وقت - نفس الاستايل", type="primary"):
    ...
    for i,t in enumerate(WATCHLIST_54):
        res=find_anytime_style(t)
        all_found.extend(res)
        prog.progress((i+1)/len(WATCHLIST_54))
    if not all_found:
        st.error("ياهو حاجب حتى الـ bid - سوي Reboot من Manage App")
    else:
        all_found.sort(key=lambda x: x[0], reverse=True)
        st.success(f"لقي {len(all_found)} عقد بأي وقت - الحيتان OI>8000 أول")
        for score, c in all_found[:30]:
            key=f"{c['ticker']}_{c['exp']}_{c['strike']}_{c['type']}_{datetime.now(RIYADH).strftime('%Y-%m-%d')}"
            if key in sent: continue
            reason=f"{c['type']} BW {c['bw']}%"
            if c['oi']>8000: reason+= " 🐋 حوت"
            msg=build_msg_old_style(c, reason)
            st.code(msg)
            if send(msg):
