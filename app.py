...
# ===== بدون scipy - نحسب اليونانيات بنفسنا =====
def norm_cdf(x):
...
st.set_page_config(page_title="حوت 54 - النهائي", layout="wide")
...
st.info(f"اليوم: {today_sa} | نيويورك: {now_et.strftime('%H:%M')} | مدمج: تاريخ+Vol+OI+يونانيات+وقت+انفجار+اليوت")
    ...
    try:
        ...
        for exp, days in valid[:2]:
            ...
            for _,row in df.iterrows():
                ...
                cand={"symbol":sym,"side":tech['direction'],"pre":pre,"curr":curr,"exp":exp,"days":days,"strike":row['strike'],"bid":float(row['bid']),"vol":int(row['volume']),"oi":int(row['openInterest']),"iv":iv,"delta":greeks['delta'],"gamma":greeks['gamma'],"theta":greeks['theta'],"vega":greeks['vega'],"spread":spread,"tech":tech,"score":final_score,"status": "🔥 طالع 90%" if final_score>=88 else "🚀 صاعد 80%" if final_score>=75 else "⏳ مراقبة"}
                if best is None or cand['score'] > best['score']: best=cand
        return best if best and best['score']>=72 else None
    except: return None

if st.button("🔍 فحص نهائي مدمج", type="primary", use_container_width=True):
    results=[]
    prog=st.progress(0)
    ...
    if not results:
        st.error("اليوم ما فيه عقد يجمع كل الشروط")
    else:
        for r in results:
            t=r['tech']
            emoji="🟢" if r['side']=="CALL" else "🔴"
            stxt=int(r['strike']) if r['strike']==int(r['strike']) else r['strike']
            msg=f"""{r['status']} {emoji} {r['symbol']} {stxt} {r['side']} PRE {r['pre']:.2f}% | Δ{abs(r['delta']):.2f} Γ{r['gamma']:.3f} Θ{r['theta']:.3f}
Exp: {r['exp']} ({r['days']}d) Stock: ${r['curr']:.2f} IV:{r['iv']*100:.0f}% Vol: {r['vol']} OI: {r['oi']}
{', '.join(t['reasons'])} | VWAP:${t['vwap']:.2f} RSI:{t['rsi']:.0f}
Target: ${r['bid']*1.8:.2f} (+80%) | ${r['bid']*2.8:.2f} (+180%) | ${r['bid']*4.5:.2f} (+350%)"""
            st.code(msg)
...
if 'res' in st.session_state and st.session_state['res']:
    if st.button("📤 ارسل لتلجرام", type="primary", use_container_width=True):
        ...
        for r in st.session_state['res']:
            msg=f"{r['status']} {r['symbol']} {r['strike']} {r['side']} Δ{abs(r['delta']):.2f} {r['days']}d"
            try:
