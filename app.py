...
def get_strong_direction(ticker, timeframe="15m", strict=True):
    try:
        tk = yf.Ticker(ticker)
        ...
        if len(df) < 30 or df_daily.empty:
            return None, None

        # استبعاد الشمعة المفتوحة - هذا حل مشكلة قبل الإغلاق
        ...
        # في الوضع العادي نسمح بتذبذب 1.5% بدل 0.8%
        live_dev = abs(curr_live - curr_closed) / curr_closed * 100 if curr_closed else 0
        if strict and live_dev > 1.5:
            return None, {"curr": curr_live, "closed": curr_closed, "dev": live_dev, "rsi": 0, "gap": 0, "spy_up": False, "reason": "تذبذب"}
        ...
        df5 = tk.history(period="5d", interval="5m", prepost=False, auto_adjust=True)
        if len(df5) > 1:
            df5 = df5.iloc[:-1]
        ...
        if strict:
            # وضع صارم - للسوق المفتوح
            call_strong = (ema9 > ema20 and ema20 > ema50 and curr_closed > vwap and curr_closed > ema200 * 0.98 and 38 < rsi_now < 75 and gap_pct > -3)
            put_strong = (ema9 < ema20 and ema20 < ema50 and curr_closed < vwap and curr_closed < ema200 * 1.02 and 25 < rsi_now < 62 and gap_pct < 3)
        else:
            # وضع توربو - يعطي نتائج حتى والسوق مقفل
            call_strong = (ema9 > ema20 and curr_closed > ema50 and curr_closed > vwap * 0.998 and 35 < rsi_now < 78)
            put_strong = (ema9 < ema20 and curr_closed < ema50 and curr_closed < vwap * 1.002 and 22 < rsi_now < 65)

        info = {"curr": curr_closed, "live": curr_live, "vwap": vwap, "ema9": ema9, "ema20": ema20, "ema50": ema50, "ema200": ema200, "rsi": rsi_now, "gap": gap_pct, "dev": live_dev, "spy_up": spy_up}
    ...
    try:
        ...
        for direction in ["CALL", "PUT"]:
            ...
            for _, row in opts.head(5).iterrows():
                ...
                if spread > 0.80:
                    continue
                result[direction] = {"strike": float(row['strike']), "bid": bid, "ask": ask, "entry": entry, "volume": int(row.get('volume', 0) or 0), "oi": int(row.get('openInterest', 0) or 0)}
                break
        return result
    except:
        return None
...
st.title("🐳 بوت الحيتان TURBO PRO")

c1, c2, c3, c4 = st.columns(4)
with c1:
    timeframe = st.selectbox("الفريم", ["5m", "15m", "30m", "1h", "1d"], index=1)
with c2:
    min_p = st.number_input("أقل سعر", 0.05, 5.0, 0.05, step=0.05)
    max_p = st.number_input("أعلى سعر", 0.10, 50.0, 20.0, step=0.10)
with c3:
    dte_min = st.number_input("DTE من", 0, 90, 0)
    dte_max = st.number_input("DTE إلى", 1, 90, 14)
with c4:
    mode = st.selectbox("الوضع", ["توربو (يعطي نتائج)", "صارم (دقيق)"], index=0)
    show_all = st.checkbox("اظهر حتى الفاشلة")

strict_mode = (mode == "صارم (دقيق)")
...
if st.button("🚀 فحص", use_container_width=True, type="primary"):
    ...
    for t in WATCHLIST:
        try:
            ...
            direction, info = get_strong_direction(t, timeframe, strict=strict_mode)
            if not direction:
                if show_all and info and info.get('rsi', 0)!= 0:
                    st.write(f"⚪ {t} ${curr_live:.2f} | RSI {info.get('rsi',0):.1f} | Gap {info.get('gap',0):+.2f}% | EMA9 {info.get('ema9',0):.2f} vs EMA20 {info.get('ema20',0):.2f}")
                continue
            if not tk.options:
                continue
            valid_exp = None
            ...
            st.write(f"❌ {t}: {e}")
            continue
    st.success(f"✅ تم ارسال {sent} عقد - الوضع: {mode}")
