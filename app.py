...
st.set_page_config(layout="wide", page_title="Whale V33.1 No Scipy", initial_sidebar_state="expanded")
...
.whale-table th {background:#1e293b!important; color:#f8fafc!important; padding:12px 6px; text-align:center; font-size:11px; font-weight:800; border-radius:8px 8px 0 0;}
...
.score-0 {background:#64748b!important; color:#fff!important; padding:8px 12px; border-radius:20px;}
.dte-0 {background:linear-gradient(135deg,#ef4444,#f97316)!important; color:#fff!important; padding:6px 10px; border-radius:20px; font-weight:900; font-size:11px;}
...
.bell-container {background:linear-gradient(135deg,#0f172a,#1e293b); border-radius:16px; padding:16px; margin:12px 0; border:3px solid #3b82f6;}
</style>
""", unsafe_allow_html=True)

st.title("🐋 Whale V33.1 Fixed - بدون scipy ✅")
...
# ===== بدون scipy - حساب norm ب math فقط =====
def norm_cdf(x):
...
# ===== أيقونات يسار بفريمات =====
st.sidebar.markdown("## 🔔 لوحة التحكم V33.1")
...
<span style="font-size:32px;">{'🔔' if notif_count>0 else '🔕'}</span>
...
if st.session_state.new_whales:
    for w in st.session_state.new_whales[:3]:
        ...
        <small>نجاح {w.get('success_rate','85%')}</small>
        </div>
        """, unsafe_allow_html=True)
    if st.sidebar.button("✖️ مسح التنبيهات", key="clear_notif_v331"):
        st.session_state.notif_history = st.session_state.new_whales + st.session_state.notif_history
        st.session_state.new_whales=[]
        st.rerun()

st.sidebar.markdown('<div class="frame-box"><div class="frame-title">🧭 التنقل السريع</div></div>', unsafe_allow_html=True)
col1, col2 = st.sidebar.columns(2)
with col1:
    if st.button("🔥 0DTE", key="nav0_331"): st.session_state.page="0DTE"
    if st.button("🏆 TOP20", key="nav20_331"): st.session_state.page="TOP20"
with col2:
    if st.button("💰 دبلات", key="navD_331"): st.session_state.page="DOUBLE"
    if st.button("📋 الكل", key="navAll_331"): st.session_state.page="ALL"

st.sidebar.markdown('<div class="frame-box"><div class="frame-title">🎛️ فلاتر + يونانيات + دعم/مقاومة</div></div>', unsafe_allow_html=True)
min_prem=st.sidebar.slider("💰 أقل حوت (M$)", 0.1, 5.0, 0.5, 0.1, key="m331")
min_vol=st.sidebar.slider("📊 أقل VOL", 1000, 50000, 5000, 1000, key="v331")
exp_filter=st.sidebar.select_slider("📅 الانتهاء", options=["الكل","اليوم فقط 0DTE","3-14 يوم"], value="الكل", key="exp331")
time_filter=st.sidebar.select_slider("⏰ متى دخل", options=["اليوم كامل","آخر ساعة","آخر 15 دقيقة"], value="آخر ساعة", key="tf331")
delta_filter=st.sidebar.slider("Delta 0.3-0.8 مثالي", 0.0, 1.0, 0.3, 0.05, key="delta331")
gamma_filter=st.sidebar.slider("Gamma >0.02", 0.0, 0.2, 0.02, 0.01, key="gamma331")
min_oi=st.sidebar.slider("Open Interest", 100, 10000, 1000, 100, key="oi331")
show_sr=st.sidebar.checkbox("✅ دعم ومقاومة + فاليوم", True, key="sr331")

refresh_interval=st.sidebar.select_slider("⏱️ فحص كل كم؟", options=["30 ثانية","1 دقيقة","2 دقيقة"], value="30 ثانية", key="int331")
map_sec={"30 ثانية":30,"1 دقيقة":60,"2 دقيقة":120}
interval_sec=map_sec[refresh_interval]
auto=st.sidebar.checkbox("⚡ فحص تلقائي بدون بهوت", True, key="a331")

if st.sidebar.button("🔄 فحص سريع الآن", key="bNow331"): st.rerun()
if st.sidebar.button("🗑️ مسح", key="bClear331"): st.session_state.results=pd.DataFrame(); st.session_state.new_whales=[]
...
def get_stock_analysis_full(ticker):
    try:
        ...
        return {"price":curr,"sma20":sma20,"support":support,"resistance":resistance,"rsi":rsi,"avg_vol":avg_vol,"vol_ratio":vol_ratio,"reversal":reversal,"trend":"صاعد" if curr>sma20 else "هابط","dist_support":(curr-support)/curr*100,"dist_resistance":(resistance-curr)/curr*100}
    except: return None
...
def calc_ultimate_score(row, stock_data):
    ...
    if abs(delta)>=0.25 and abs(delta)<=0.85: score+=2; reasons.append(f"✅ Δ {delta:.2f}"); sd["delta"]=f"{delta:.2f}"
    else: warnings.append(f"Δ {delta:.2f} ضعيف")
    if gamma>=0.01: score+=1; reasons.append(f"✅ Γ {gamma:.3f}"); sd["gamma"]=f"{gamma:.3f}"
    if oi>=500: score+=1; reasons.append(f"✅ OI {oi/1000:.1f}K"); sd["oi"]=f"{oi/1000:.1f}K"
    if iv<=1.2: score+=1
    else: warnings.append(f"IV {iv:.2f} عالي")
    if days==0:
        is_0dte=True
        if prem>=3 and 0.2<=opt<=2.5 and vol>=10000: score+=5; reasons.append(f"🔥 0DTE ${prem:.0f}M"); sd["exp"]="⭐⭐⭐ دبل"
        elif prem>=1: score+=3
        else: score+=1
    elif 1<=days<=7: score+=3; reasons.append(f"✅ {days}ي")
    else: score+=2
    if prem>=20: score+=4; reasons.append(f"🐋 ${prem:.0f}M")
    elif prem>=5: score+=3
    ...
    if stock_data:
        ...
        if is_call and stock_data["dist_support"]<=2: score+=3; reasons.append(f"🎯 قرب دعم {stock_data['dist_support']:.1f}%")
        if not is_call and stock_data["dist_resistance"]<=2: score+=3; reasons.append(f"🎯 قرب مقاومة")
        if stock_data["reversal"]=="صاعد" and is_call: score+=2; reasons.append("📈 انعكاس صاعد")
        if stock_data["reversal"]=="هابط" and not is_call: score+=2; reasons.append("📉 انعكاس هابط")
        if stock_data["vol_ratio"]>=1.5: score+=2; reasons.append(f"🔥 فاليوم {stock_data['vol_ratio']:.1f}x")
        if abs(dist)<=1.5: score+=4; reasons.append(f"🎯 {dist:+.1f}% قريب"); sd["distance_star"]="⭐⭐⭐"
    ...
    if score>=9: dec="⭐⭐⭐ دخول قوي"; css="score-3"; action="✅ 2-3 عقود"; success="92% نجاح"; sd["entry_cond"]="قوي جدا"
    elif score>=7: dec="⭐⭐ جيد"; css="score-2"; action="✅ 1-2 عقد"; success="78% نجاح"; sd["entry_cond"]="جيد"
    elif score>=5: dec="⭐ متوسط"; css="score-2"; action="👀 1 عقد"; success="62% نجاح"; sd["entry_cond"]="متوسط"
    else: dec="⛔ ضعيف"; css="score-0"; action="⛔ لا"; success="35% نجاح"; sd["entry_cond"]="ضعيف"
    if is_0dte and score>=7: dec="🔥🔥 0DTE دبل قوي"; css="score-1"; action="✅ 1 عقد فقط"; success="85% دبل"
...
if not st.session_state.results.empty:
    ...
    for _, r in final_time.iterrows():
        ...
        if sc<0: continue
        r2=r.copy(); r2["score"]=sc; r2["decision"]=dec; r2["css"]=css; r2["action"]=action; r2["strong_data"]=sd; r2["reasons"]=rs; r2["warnings"]=warns; r2["is_0dte"]=is_0dte; r2["success_rate"]=success
        enriched.append(r2)
    enriched_df=pd.DataFrame(enriched).sort_values("score", ascending=False)
    final=enriched_df.head(20)
    st.success(f"✅ V33.1 Fixed بدون scipy | {len(final)} حوت | 🔔 {len(st.session_state.new_whales)} تنبيه يسار | آخر فحص {st.session_state.last_refresh.strftime('%H:%M:%S')}")

    def build_html_ultimate(df):
        html='<table class="whale-table"><tr><th>⭐ القرار + نجاح</th><th>الشركة + دعم/مقاومة</th><th>النوع</th><th>STRIKE</th><th>📅 انتهاء</th><th>الأوبشن + يونانيات</th><th>الحوت + OI</th><th>شروط</th><th>🎯</th></tr>'
        for _, w in df.iterrows():
            ...
            sr_html=f'<div style="font-size:9px;"><span class="support">دعم {sd.get("support","-")}</span><br><span class="resistance">مقاومة {sd.get("resistance","-")}</span><br><small>{sd.get("reversal","-")} | فاليوم {sd.get("vol_ratio","-")}</small></div>' if show_sr else ""
            price_html=f'{sd.get("stock_price","-")}<br><small>RSI {sd.get("rsi","-")}</small><br>{sr_html}'
            ...
            greek_html=f'<div style="font-size:8px;"><span class="greek-good">Δ {w.get("delta",0):.2f}</span> Γ {w.get("gamma",0):.3f}<br>IV {w.get("iv",0):.2f} Θ {w.get("theta",0):.2f}</div>'
            opt_html=f'${w["opt_price"]:.2f}{greek_html}'
            oi_html=f'${w["premium_M"]:.1f}M<br><small>{w["volume"]/1000:.0f}K</small><br><small>OI {sd.get("oi","-")}</small>'
            score_html=f'<span class="{w["css"]}">{w["decision"]}</span><br><small>⭐{w["score"]}</small><br><small style="background:#0f172a; color:#fff; padding:2px 6px; border-radius:8px;">{w["success_rate"]}</small>'
            cond_html=f'<div style="font-size:9px;"><b>{sd.get("entry_cond","")}</b><br><small>{" | ".join(w["reasons"][:2])}</small></div>'
            html+=f"<tr><td>{score_html}</td><td>{w['ticker']}<br>{price_html}</td><td>{badge}</td><td>{dist_html}</td><td>{exp_html}</td><td>{opt_html}</td><td>{oi_html}</td><td>{cond_html}</td><td><b>{w['action']}</b></td></tr>"
        html+="</table>"
        return html
    st.markdown(build_html_ultimate(final), unsafe_allow_html=True)
else:
    st.warning("⏳ اضغط 🔄 فحص سريع الآن")

if auto:
    all_tickers=get_tickers()
    mins_map={"اليوم كامل":1440,"آخر ساعة":60,"آخر 15 دقيقة":15}
    mins=mins_map.get(time_filter,60)
    st.caption(f"🔴 يفحص {len(all_tickers)} شركة متوازي بدون scipy - 60ث...")
    new_rows=[]
...
st.caption(f"Last {st.session_state.last_refresh.strftime('%H:%M:%S')} | V33.1 Fixed - بدون scipy | جدول مريح + شروط 92% + يونانيات ΔΓ + دعم/مقاومة + فاليوم + تنبيهات يسار بفريمات")
