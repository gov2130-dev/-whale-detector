...
st.set_page_config(layout="wide", page_title="V47 AUTO", initial_sidebar_state="collapsed")
    ...
    try:
        ...
        for i in range(1, len(df)):
            if df['Close'].iloc[i] > st_line.iloc[i-1]:
                st_line.iloc[i] = max(float(lower.iloc[i]), float(st_line.iloc[i-1])) if direction.iloc[i-1]==1 else float(lower.iloc[i])
                direction.iloc[i] = 1
            else:
                st_line.iloc[i] = min(float(upper.iloc[i]), float(st_line.iloc[i-1])) if direction.iloc[i-1]==-1 else float(upper.iloc[i])
                direction.iloc[i] = -1
...
st.markdown(f'<div class="time-card">● {ksa_str} KSA | V47 يفحص لحاله | بدون 95% وهمي</div>', unsafe_allow_html=True)
...
def analysis_v47(ticker):
    try:
        ...
        if vol_sum>0: vwap=float((h20['Close']*h20['Volume']).sum()/vol_sum)
        else: vwap=curr
        d=h['Close'].diff()
        ...
        return {"price":curr,"ema9":ema9,"ema21":ema21,"ema50":ema50,"vwap":vwap,"rsi":float(rsi),"fib_236":float(fib_236),"vol_ratio":float(vol_ratio),"change_1d":float(change_1d),"change_5d":float(change_5d),"st_dir":int(st_direction),"macd_bull":bool(macd_bull),"bounce_fib":bool(bounce_fib)}
    except: return None

def fetch_v47(ticker, min_prem, min_vol):
    try:
        tk=yf.Ticker(ticker)
        if not tk.options: return []
        sd=analysis_v47(ticker)
        if not sd: return []
        curr=sd["price"]
        ...
        for exp in tk.options[:2]:
            try:
                ...
                if sd["st_dir"]==1 and is_bull: allowed=["CALL"]
                elif sd["st_dir"]==-1 and is_bear: allowed=["PUT"]
                else:
                    if sd["change_1d"]>=1.2: allowed=["CALL"]
                    elif sd["change_1d"]<=-1.2: allowed=["PUT"]
                    else: continue
...
if st.session_state.results.empty:
    ...
    with st.spinner(f"⚡ فحص تلقائي {len(tickers)}..."):
        ...
        with ThreadPoolExecutor(max_workers=10) as executor:
            futs={executor.submit(fetch_v47, t, min_prem, min_vol): t for t in tickers}
...
st.caption(f"V47 AUTO | {ksa_str} | يفحص لحاله عند الفتح - يحل مشكلة الفاضي في صورتك")
