...
st.set_page_config(layout="wide", page_title="V108 ANY COMPANY")
...
def find_any_company(ticker):
    try:
        curr, tk = get_curr(ticker)
        if curr==0: return []
            ...
            for direction, opts in [("CALL", chain.calls), ("PUT", chain.puts)]:
                ...
                for _, r in opts.iterrows():
                    try:
                        ...
                        # شروطك انت - اي شركة تحققها تطلع
                        if oi < 8000: continue
                        if not (0.5 <= last <= 5.0): continue
                        if bw > 2.5: continue
                        best.append((oi, {"ticker":ticker,"curr":curr,"exp":exp,"days":days,"strike":strike,"last":last,"bid":bid,"vol":int(r.get('volume',0) or 0),"oi":oi,"type":direction,"bw":round(bw,2)}))
                    except: continue
            time.sleep(0.7)
        best.sort(key=lambda x: x[0], reverse=True)
        return [c for oi,c in best[:3]] # اقوى 3 لكل شركة عشان ما يصير 70 في SPX
    except: return []

# نفس استايل رسالتك اللي صورتها بالضبط
def build_msg_same_style(c):
    ...
    return f"{emoji} {c['ticker']} {int(c['strike'])} {c['type']} {mode} - {c['type']} BW {c['bw']}%\nExp: {c['exp']} ({c['days']}d) Stock: ${base:.2f}\nEntry: ${last:.2f} Bid: ${c['bid']:.2f} Vol: {c['vol']} OI: {c['oi']}\nStop: ${last*0.55:.2f}\nTarget Stock: {t1:.1f} > {t2:.1f} > {t3:.1f}\nTarget Contract: ${last*1.5:.2f} (+50%) | ${last*2.3:.2f} (+130%) | ${last*3.2:.2f} (+220%)"

st.title("V108 - اي شركة تحقق الشروط بنفس الاستايل")
st.caption("بحث بأي وقت | OI>8000 BW<2.5% 0.5-5$ | اي شركة من 54 تطلع")
col1,col2=st.columns(2)
with col1:
    if st.button("📨 اختبار تلجرام"): st.success("✅") if send("✅ V108 شغال بنفس استايلك") else st.error("فشل")
with col2:
    if st.button("🗑️ تصفير المرسلة"): save([]); st.success("تصفر")
    st.metric("المرسلة اليوم", len(load()))
sent=load()

if st.button("🔍 افحص 54 - اي شركة تحقق الشروط", type="primary"):
    prog=st.progress(0)
    all_found=[]
    for i,t in enumerate(WATCHLIST_54):
        res=find_any_company(t)
        if res: all_found.extend(res)
        prog.progress((i+1)/len(WATCHLIST_54))
    if not all_found:
        st.warning("ما فيه اي شركة تحقق OI>8000 + BW<2.5% + 0.5-5$ الان - السوق ما فيه حوت الان")
    else:
        all_found.sort(key=lambda x: (-x['oi'], x['bw']))
        st.success(f"لقي {len(all_found)} عقد - اي شركة حققت شروطك")
        for c in all_found:
            key=f"{c['ticker']}_{c['exp']}_{c['strike']}_{c['type']}_{datetime.now(RIYADH).strftime('%Y-%m-%d')}"
            if key in sent: continue
            msg=build_msg_same_style(c)
            st.code(msg)
            if send(msg):
