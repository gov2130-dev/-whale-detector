...
st.set_page_config(layout="wide", page_title="Whale V30 No Flicker Final", initial_sidebar_state="expanded")
...
st.title("🐋 Whale V30 - بدون بهوت نهائيا ✅ ثابت 100%")

def get_tickers():
    return ["SPY","QQQ","TSLA","NVDA","AAPL","AMZN","META","MSFT","GOOGL","AMD","NFLX","COIN","MSTR","PLTR","GME","MARA","SOFI","NIO","JPM","BAC","XOM","LLY","AVGO","ARM","GLD","IWM","SMH","TNA"]
...
# ===== بدون بهوت - شلت كل أسباب الوميض =====
st.sidebar.markdown("## 🎯 V30 - بدون بهوت نهائيا")

st.sidebar.success("✅ شلت st.rerun() و components.html اللي كان يسبب البهوت")

st.sidebar.markdown("### 💰 فلاتر")
min_prem=st.sidebar.slider("💰 أقل حوت", 100000, 5000000, 500000, 100000, key="m30")
min_vol=st.sidebar.slider("📊 أقل VOL", 1, 50000, 5000, 1000, key="v30")

trade_mode=st.sidebar.select_slider("🎯 نمطك؟", options=["سكالبينج 0DTE (دبلات)","سوينغ 3-14 يوم","الكل - 0DTE = دبل"], value="الكل - 0DTE = دبل", key="mode30")
exp_filter=st.sidebar.select_slider("📅 الانتهاء", options=["الكل (0DTE دبل)","اليوم فقط 0DTE","3-14 يوم"], value="الكل (0DTE دبل)", key="exp30")
time_filter=st.sidebar.select_slider("⏰ متى دخل", options=["اليوم كامل","آخر ساعة","آخر 15 دقيقة"], value="آخر ساعة", key="tf30")

st.sidebar.markdown("---")
st.sidebar.markdown("### 📊 عرض")
if st.sidebar.button("🔥 0DTE اليوم - دبلات", key="b0_30"): st.session_state.page="0DTE"
if st.sidebar.button("💰 دبلات محتملة", key="bDouble_30"): st.session_state.page="DOUBLE"
if st.sidebar.button("🏆 أقوى 20", key="b20_30"): st.session_state.page="TOP20"
if st.sidebar.button("🐋 أقوى 10", key="b10_30"): st.session_state.page="TOP10"
if st.sidebar.button("📋 كل اليوم", key="bAll_30"): st.session_state.page="ALL"

st.sidebar.markdown("---")
# زر واحد فقط - بدون تحديث تلقائي يسبب بهوت
if st.sidebar.button("🔄 فحص السوق الآن - الثلاثاء", key="bNow_30"):
    with st.spinner("🔍 يفحص السوق... (10 ثواني بدون بهوت)"):
        # فحص كامل مرة واحدة بدون rerun
        ...
        for i, t in enumerate(all_tickers):
            try:
                ...
                if not s.options:
                    ...
                    continue
                exp=s.options[0]
                # جرب أول تاريخين فقط عشان السرعة
        ...
        if new_rows:
            ...
            st.sidebar.success(f"✅ لقى {len(new_df)} حوت جديد!")
        else:
            st.sidebar.warning("ما لقى - نزل الفلتر لـ 100k")

if st.sidebar.button("🗑️ مسح", key="bClear_30"):
...
def calc_score(row, stock_data):
    ...
    if days<0: return -10, "⛔ منتهي", "score-0", "⛔", {}, [], [], False
    ...
    if days==0:
        ...
        warnings.append("⚠️ 1 عقد فقط")
    elif 1<=days<=7: score+=3; reasons.append(f"✅ {days}ي"); sd["exp"]=f"{days}ي"
    else: score+=2; reasons.append(f"✅ {days}ي")
    ...
    if stock_data:
        ...
        if is_call and stock_data["trend"]=="صاعد" and stock_data["rsi"]<70: score+=2; reasons.append(f"📈 صاعد RSI {stock_data['rsi']:.0f}")
...
if not st.session_state.results.empty:
    ...
    if final_time.empty:
        st.warning(f"لا يوجد في {st.session_state.page} - أعرض الكل")
    ...
    st.success(f"✅ V30 ثابت بدون بهوت | {st.session_state.page} | {len(final)} حوت | آخر فحص {st.session_state.last_refresh.strftime('%H:%M:%S')} | الثلاثاء مباشر")

    def build_html(df):
        html='<table class="whale-table"><tr><th>⭐ القرار</th><th>الشركة + السعر</th><th>النوع</th><th>STRIKE + المسافة</th><th>📅 انتهاء</th><th>الأوبشن</th><th>الحوت</th><th>دبل؟</th><th>ادخل؟</th></tr>'
    ...
    st.info("""
    **✅ حل البهوت نهائيا V30:**
    - شلت `st.rerun()` التلقائي اللي كان يسبب وميض كل ثانية
    - شلت `components.html` للجرس اللي كان يهز الشاشة
    - الآن الفحص يدوي بزر واحد `🔄 فحص السوق الآن` - يفحص 10 ثواني بدون بهوت والجدول ثابت
    - تقدر تقرأ وتضغط براحتك بدون ما الشاشة ترمش
    - 0DTE منطقي: رخيص + قريب = دبل، مع 1 عقد فقط
    """)
else:
    st.warning("⏳ اضغط باليسار **🔄 فحص السوق الآن - الثلاثاء** - أول مرة فقط")
    st.info("💡 V30 بدون بهوت: ما فيه تحديث تلقائي يهز الشاشة. كل فحص بزر يدوي ثابت 100%")

st.caption(f"Last {st.session_state.last_refresh.strftime('%H:%M:%S')} | V30 No Flicker Final | ثابت بدون بهوت")
